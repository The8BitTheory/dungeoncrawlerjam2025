...
50 W=28:DIM RL%(5,2),M%(28*31-1),V%(3,19),P%(7),GS%(1),TL%(1),MP%(2),PP%(4),FD%(3)
...
65  IF MV$="6" THEN PP%(0)=X:ELSE IF MV$="8" THEN TL%(TL)=X:TL=TL+1
66  MV=ASC(MV$)-48
...
74 IF PP%(0)=0 THEN PRINT "NO STARTPOINT SET ON MAP":STOP
...
10026 DEF FN M(ZZ)=M%(PP%(0)+V%(FD%,ZZ))
10027 DEF FN FD(ZZ)=INT(FD%+ZZ+4)-INT(INT(FD%+ZZ+4)/INT(4))*INT(4)
10028 DEF FN PP(ZZ)=PP%(0)+P%(FD%+FD%+ZZ)
