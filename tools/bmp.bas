...
100 BLOAD "VDCBASIC2D.0AC6",B0,U(DD):SYS DEC("AC6")
...
596 PRINT " COPY "VP"-"(VP+FL)" TO "(DB+IS+2)", LEN "FL

599 VTR VP,DB+IS+2,FL

#   COPY FROM INVISIBLE TO VISIBLE VRAM
#600 VMC VP,16000,BW+1,BH,80,BW+1


602 POKE DB+IS,BW:POKE DB+IS+1,BH

604 WF$="@"+LEFT$(PF$,LEN(PF$)-4)+".VDC"
605 BSAVE (WF$),B0,U(DD),P(DB+IS) TO P(DB+IS+FL+2)
